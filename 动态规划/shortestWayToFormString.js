...
// 示例 3：
// 输入：source = "xyz", target = "yxz"
// 输出：2
// 解释：目标字符串可以按如下方式构建： "y" + "xz"。

// 提示：
// source 和 target 两个字符串都只包含 "a"-"z" 的英文小写字母。
// source 和 target 两个字符串的长度介于 1 和 1000 之间。
let shortestWayToFormString = function (source, target) {
    let len = target.length;
    let start = 0, end = 0, i = 0;
    let num = 0;
    while (i < len){
        while (target[i] !== source[start]) {
            start++;
...
let result = shortestWayToFormString("abc", "acdbc");
console.log(result);
